...
# Standard implementation of get_commands() to ensure consistency
...
# Cog files and their command groups
COG_GROUPS = {
    'server_commands_slash.py': 'server_group',
    'stats_commands.py': 'stats_group',
    'killfeed_commands.py': 'killfeed_group',
    'connection_commands.py': 'connection_group',
    'mission_commands.py': 'mission_group',
    'faction_commands.py': 'faction_group',
    'admin_commands.py': 'admin_group'
}
...
def check_and_fix_cogs():
    ...
    for filename, group_name in COG_GROUPS.items():
        file_path = cog_dir / filename
        
        if not file_path.exists():
            logger.warning(f"Cog file not found: {file_path}")
            continue
            
        # Check if get_commands is already implemented
        file_content = file_path.read_text()
        
        if 'def get_commands(self)' in file_content:
            logger.info(f"✅ {filename}: Already has get_commands() method")
            continue
            
        # Find the insertion point - after cog_load and before the first command
        lines = file_content.splitlines()
        insert_after = -1
        
        # Look for cog_load first
        for i, line in enumerate(lines):
            if 'async def cog_load' in line:
                # Find the end of the cog_load method
                for j in range(i+1, len(lines)):
                    if not lines[j].strip() or lines[j].startswith('    #') or lines[j].startswith('    def') or lines[j].startswith('    @'):
                        insert_after = j - 1
                        break
        ...
        # If we didn't find cog_load, look for __init__ end
        if insert_after < 0:
            for i, line in enumerate(lines):
        ...
        # If we still don't have an insertion point, use a more general approach
        if insert_after < 0:
            for i, line in enumerate(lines):
                if line.strip() == 'class' and 'Cog' in line:
                    # Find first method after class definition
                    for j in range(i+1, len(lines)):
        ...
        # If we still don't have an insertion point, use line 20 as a fallback
        if insert_after < 0:
            insert_after = min(20, len(lines) - 1)
        
        # Insert the get_commands method
        fixed_method = GET_COMMANDS_TEMPLATE.format(group_name=group_name)
        new_lines = lines[:insert_after+1] + fixed_method.splitlines() + lines[insert_after+1:]
        
        # Write back to file
        file_path.write_text('\n'.join(new_lines))
        logger.info(f"✅ {filename}: Added get_commands() method")
        fixed_count += 1
...
def update_sync_retry_module():
    ...
    # Path to the sync_retry module
    sync_retry_path = Path('utils/sync_retry.py')
    
    # Check if the module already exists
    if not sync_retry_path.parent.exists():
        logger.info("Creating utils directory...")
        sync_retry_path.parent.mkdir(parents=True, exist_ok=True)
    
    # Implementation from command_fix_implementation.py
    sync_retry_content = """
\"\"\"
...
"""
    ...
    # Write to file
    sync_retry_path.write_text(sync_retry_content.strip())
    logger.info("✅ Updated sync_retry module")
    
    return True

async def force_command_registration():
    """Force command registration for all commands"""
    logger.info("Updating .last_command_sync file to force registration...")
    
    # Create current timestamp in .last_command_sync file
    last_sync_file = Path('.last_command_sync')
    with open(last_sync_file, 'w') as f:
        f.write(str(time.time()))
    
    logger.info("✅ Updated .last_command_sync - bot will register commands on next restart")
    
    return True

def main():
    """Execute all command fixes in a unified approach"""
    logger.info("Starting unified command fix implementation")
    
    # Step 1: Fix missing get_commands() methods in cogs
    fixed_count = check_and_fix_cogs()
    if fixed_count > 0:
        logger.info(f"Fixed {fixed_count} cogs with missing get_commands() methods")
    else:
        logger.info("All cogs already have get_commands() methods")
    
    # Step 2: Update sync_retry module
    update_result = update_sync_retry_module()
    if update_result:
        logger.info("Successfully updated sync_retry module")
    else:
        logger.warning("Failed to update sync_retry module")
    
    # Step 3: Force command registration on next restart
    asyncio.run(force_command_registration())
    
    logger.info("✅ Command fix implementation complete!")
    logger.info("The bot will automatically register all commands on the next restart.")
